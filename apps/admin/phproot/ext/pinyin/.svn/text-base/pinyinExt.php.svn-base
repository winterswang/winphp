    ...
    public function getPinyin($str,$split='',$firstUpper=0,$ishead=0)
    {
        ...
        for($i=0; $i<$slen; $i++)
        {
            if(ord($str[$i])>0x80)
            {
                $c = $str[$i].$str[$i+1];
                $i++;
                if(isset(self::$pinyinData[$c]))
                {
                    if($ishead==0)
                    {
                        $restr .= $split.($firstUpper ? ucfirst(self::$pinyinData[$c]) : self::$pinyinData[$c] );
                    }
                    else
                    {
                        $restr .= $split.($firstUpper ? ucfirst( self::$pinyinData[$c][0] ) : self::$pinyinData[$c] );
                    }
                }else
                {
                    $restr .= "_";
