...
class Image_ImageMagick_Driver extends Image_Driver {
	...
	{
		if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' '.$cmdline))
		{
			$this->errors[] = $error;
			return FALSE;
		}
	...
	public function blur($prop)
	{
		// Sanitize and normalize the properties into geometry
		$this->sanitize_geometry($prop);

		// Set the IM geometry based on the properties
		$cmdline = 	$prop['sigma'] ? $prop['quality'].'x'.$prop['sigma'] : $prop['quality'];

		$geometry = escapeshellarg($cmdline);

		if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' -blur '.$geometry.' '.$this->cmd_image.' '.$this->cmd_image))
		{
			$this->errors[] = $error;
			return FALSE;
		}
	...
	{
		$geometry = escapeshellarg('+'.$prop['offset_y'].'+'.$prop['offset_x']);

		if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' '.$this->cmd_image.' '.$prop['watermark'].' -gravity southeast -geometry '.$geometry.' -composite '.$this->cmd_image ))
		{
			$this->errors[] = $error;
			return FALSE;
		}
