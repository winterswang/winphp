...
class xuequ_district extends BaseDb{
    ...
    public function getDistrictNameList($where = array()){
        $sql = "select district_name from xuequ_district". $this->buildWhere($where);
		return $this->fetch_all($sql);    	
    }
	public function getList($where = array(),$page_no = 1, $page_size = 10)
	{
        $sql = "select * from xuequ_district". $this->buildWhere($where).$this->limit($page_no, $page_size);;
		return $this->fetch_all($sql);
	}
