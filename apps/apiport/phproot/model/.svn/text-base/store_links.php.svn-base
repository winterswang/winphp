...
class store_links extends BaseDb{
	...
	public function getFlush(){
		$minId = $this->getMinId(array('analyze_status'=>0));
		if(empty($minId)){
			return null;
		}
	...
	public function getMinId($where =array()){
		$sql = "select min(id) from store_links".$this->buildWhere($where);
		$data = $this->fetch($sql);
