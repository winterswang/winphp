<?php
class district_homelink extends BaseDb{
	...
	public function getCount($where = array()){
		$sql = "SELECT count(district_id) as count FROM district_homelink ".$this->buildWhere($where);
		$row = $this->fetch($sql);
		return $row['count'];
	}
	...
	public function updateHouseCount(){
		$sql = "update district_homelink as d,(select district_id,count(house_guid) as cn from house group by district_id) as t set d.house_count = t.cn where d.district_id = t.district_id ";
		return $this->fetch($sql);
	}
	public function getInfoById($guid)
	{	
		$sql = "SELECT * FROM district_homelink ".$this->buildWhere(array('id' =>$guid ));
		return $this->fetch($sql);
	}

	public function getSimpleInfo($where = array())
	{
		$sql = "SELECT * FROM district_homelink ".$this->buildWhere($where);
		return $this->fetch($sql);
	}
    ...
    public function getDistrictId($where = array(),$page_no = 1, $page_size = 10){
		$sql = "SELECT district_id FROM district_homelink ".$this->buildWhere($where).'ORDER BY district_id ASC';
	...
	{
        $sql = "select * from district_homelink". $this->buildWhere($where) .$this->limit($page_no, $page_size);
		return $this->fetch_all($sql);
	}
	...
	public function getAll($where = array(),$limit = 0)
	{
        ...
        $sql = "select * from district_homelink". $this->buildWhere($where) . $this->order($orderby).$limit_str;
		return $this->fetch_all($sql);
	}

	public function getMaxDistrictGuid(){
		$sql = "select max(district_guid) from district_homelink";
		$data = $this->fetch($sql);
    ...
    public function addDistrict($arr){
		...
		if(!$this->insert('district_homelink',$arr)){
			$this->getDb()->rollBack();
		}
	...
	{
        return $this->update('district_homelink',$arr, $where);
	}
