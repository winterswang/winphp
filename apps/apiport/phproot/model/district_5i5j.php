<?php
class district_5i5j extends BaseDb{
	...
	public function getCount($where = array()){
		$sql = "SELECT count(district_id) as count FROM district_5i5j ".$this->buildWhere($where);
		$row = $this->fetch($sql);
		return $row['count'];
	}
	...
	public function updateHouseCount(){
		$sql = "update district_5i5j as d,(select district_id,count(house_guid) as cn from house group by district_id) as t set d.house_count = t.cn where d.district_id = t.district_id ";
		return $this->fetch($sql);
	}
	public function getInfoById($guid)
	{	
		$sql = "SELECT * FROM district_5i5j ".$this->buildWhere(array('id' =>$guid ));
		return $this->fetch($sql);
	}

	public function getSimpleInfo($where = array())
	{
		$sql = "SELECT * FROM district_5i5j ".$this->buildWhere($where);
		return $this->fetch($sql);
	}
    ...
    public function getDistrictId($where = array(),$page_no = 1, $page_size = 10){
		$sql = "SELECT district_id FROM district_5i5j ".$this->buildWhere($where).'ORDER BY district_id ASC';
	...
	{
        $sql = "select * from district_5i5j". $this->buildWhere($where) .$this->limit($page_no, $page_size);
		return $this->fetch_all($sql);
	}
	...
	public function getAll($where = array(),$limit = 0,$orderby = 'createtime_desc')
	{
        ...
        $sql = "select * from district_5i5j". $this->buildWhere($where) . $this->order($orderby).$limit_str;
		return $this->fetch_all($sql);
	}

	public function getMaxDistrictGuid(){
		$sql = "select max(district_guid) from district_5i5j";
		$data = $this->fetch($sql);
    ...
    public function addDistrict($arr){
		...
		if(!$this->insert('district_5i5j',$arr)){
			$this->getDb()->rollBack();
		}
	...
	{
        return $this->update('district_5i5j',$arr, $where);
	}
